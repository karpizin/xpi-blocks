...
class SonarPatternAnalyzerNode(Node):
    """
    ROS2 Node that analyzes simple patterns in sonar data using an LLM.
    Subscribes to sensor_msgs/Range and publishes std_msgs/String with LLM analysis.
    """

    def __init__(self):
        super().__init__('sonar_pattern_analyzer_node')
        ...
        self.declare_parameter('history_size', 30) # More data for pattern detection
        self.declare_parameter('analysis_interval_sec', 10.0) # How often to analyze
        self.declare_parameter('llm_temperature', 0.5)
        self.declare_parameter('llm_max_tokens', 150)
        ...
        self.analysis_publisher = self.create_publisher(String, '~/sonar_pattern', 10)

        # 5. Analysis Timer
        self.analysis_timer = self.create_timer(self.analysis_interval, self.analyze_pattern_callback)
    ...
    def analyze_pattern_callback(self):
        """Callback to trigger LLM analysis of sonar patterns."""
        if not self.llm_client:
            self.get_logger().warn('LLM client not initialized, skipping pattern analysis.')
            return

        with self.history_lock:
            if len(self.sonar_history) < self.history_size / 2: # Need enough data
                # self.get_logger().info('Not enough sonar data for pattern analysis.')
                return
        ...
        prompt = (
            ...
            f"Analyze the following sequence of sonar readings for simple, repeating, or notable patterns. "
            f"Examples: 'object at fixed distance', 'object moved from X to Y', 'object appeared/disappeared', 'ping-pong pattern', 'no clear pattern'.\n"
            f"Sonar readings ({len(formatted_readings)} points):\n"
            f"{'; '.join(formatted_readings)}\n"
            f"What simple patterns do you observe? Be concise. Start directly with the pattern description."
        )
    ...
    def _call_llm_for_analysis(self, prompt: str):
        ...
        try:
            ...
            llm_response, _ = self.llm_client.generate( # No tools needed here
                prompt,
                temperature=self.llm_temperature,
                max_tokens=self.llm_max_tokens
            )
            if llm_response:
                self.get_logger().info(f'LLM Pattern Analysis: {llm_response}')
                msg = String()
            ...
            self.get_logger().error(f'Error during LLM pattern analysis: {e}')

def main(args=None):
    rclpy.init(args=args)
    node = SonarPatternAnalyzerNode()
    try:
        rclpy.spin(node)
