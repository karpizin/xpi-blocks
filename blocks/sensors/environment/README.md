...
This block provides a ROS2 driver for the BMP085 or BMP180, a digital sensor for measuring atmospheric pressure and ambient temperature. It publishes `sensor_msgs/Temperature` and `sensor_msgs/FluidPressure` messages.

## 📦 Bill of Materials
*   Raspberry Pi
*   BMP085/BMP180 module (I2C interface)
*   Jumper Wires

## 🔌 Wiring
Connect the BMP085/BMP180 module to the I2C pins on the Raspberry Pi.

| BMP Pin | Raspberry Pi | Note |
|---------|--------------|-----------------------------------|
| VCC     | 3.3V         | Power for the module (Pin 1)      |
| GND     | GND          | Common Ground (Pin 6)             |
| SDA     | GPIO 2 (SDA) | Data line (Pin 3)                 |
...
1.  **Enable I2C:**
    ...
    # Interface Options -> P3 I2C -> Yes
    ```
2.  **Verify I2C Address:** Check the BMP085/180's I2C address (usually 0x77)
    ```bash
    sudo apt install i2c-tools
    i2cdetect -y 1 # Or 0 if using an older Pi
    # Look for '77' in the output.
    ```

## 🚀 Quick Start
1.  **Perform I2C Configuration** as described above.
2.  **Launch the BMP085 driver**:
    ```bash
    ros2 launch xpi_sensors bmp085.launch.py i2c_address:=0x77
    ```
...
### Parameters
*   `i2c_bus` (int, default: `1`): I2C bus number.
*   `i2c_address` (int, default: `0x77`): I2C address of the BMP085/180.
*   `publish_rate` (float, default: `1.0`): Frequency to publish data in Hz.
*   `oss` (int, default: `0`): Oversampling Setting (0-3). Higher values mean higher resolution but slower conversion.
*   `frame_id` (string, default: `bmp085_link`): Frame ID for messages.
*   `mock_hardware` (bool, default: `false`): Run in mock mode without real BMP085/180 hardware.

## ✅ Verification
1.  Launch the driver with your BMP085/180 connected and I2C enabled.
2.  In new terminals, monitor the published topics:
    ```bash
    ros2 topic echo /bmp085_env_sensor/temperature
    ros2 topic echo /bmp085_env_sensor/pressure
    ```
    You should see streams of environmental data.

## ⚠️ Troubleshooting
*   **"Failed to initialize BMP085" / "I/O Error"**:
    *   Double-check I2C wiring (SDA/SCL, VCC/GND).
    *   Verify I2C is enabled (`sudo raspi-config`).
    *   Ensure the correct `i2c_address` is used.
    *   Check for I2C permissions (`sudo usermod -a -G i2c $USER`).
