...
ros2 topic pub /hexapod/body_pose geometry_msgs/Pose "{position: {z: -0.05}}"
```

### Стабилизация
Для работы авто-левелера необходимо, чтобы узел IMU публиковал данные в `/imu/data`. Узел `auto_leveler_node` включен в `hexapod_full.launch.py` по умолчанию.
